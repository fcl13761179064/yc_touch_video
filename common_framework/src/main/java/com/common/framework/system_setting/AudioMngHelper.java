package com.common.framework.system_setting;
...
public class AudioMngHelper {
    ...
    private int VOICE_STEP_100 = 5; //0-100的步进。
    ...
    public void setStreamMute(boolean state) {
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
            ...
            audioManager.adjustStreamVolume(NOW_AUDIO_TYPE, direction, 0);
        } else {
            audioManager.setStreamMute(NOW_AUDIO_TYPE, state);
        }
    }

    public boolean isStreamMute() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            return audioManager.isStreamMute(NOW_AUDIO_TYPE);
        } else {
            return false;
        }
    }
